...
class Patho(models.Model):
    ...
    raw = models.CharField(max_length=320)  # raw name
    name = models.CharField(max_length=320)  # prettified name
    delimiters = models.CharField(max_length=64,
                                  null=True)  # encoded delimiters

    def __str__(self):
        return self.name

    def get_delimiters(self):
        """Get list of int from encoded to string delimiters.
        ex: self.delimiters == '1 10 20'
        """
        if self.delimiters:
            return [int(i) for i in self.delimiters.split()]


class Line(models.Model):

    CODE_KIND_CHOICES = [('INTAKE', 'Intake'), ('REPORT', 'Report'),
                         ('CONSULT', 'Consultative'), ('DOUBLE', 'Double'),
                         ('SECOND', 'Second'),
                         ('MLD', 'Manual Lymphatic Drainage'),
                         ('STANDARD', 'Others')]

    location = models.ForeignKey("timogen.location",
                                 related_name="lines",
                                 related_query_name="line",
                                 on_delete=models.CASCADE,
                                 null=True)
    patho = models.ForeignKey("timogen.patho",
                              related_name="lines",
                              related_query_name="line",
                              on_delete=models.CASCADE,
                              null=True)
    ...
    def get_all_fees(self):
        return [
            self.fees, self.bfees_c_p, self.bfees_nc_p, self.bfees_c_np,
            self.bfees_nc_np
        ]

    def get_bfees(self, contracted, privileged):
        field_name = 'bfees_%sc_%sp' % ('n' * (not contracted), 'n' *
                                        (not privileged))
