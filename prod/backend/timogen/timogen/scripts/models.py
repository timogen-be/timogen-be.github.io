...
class NomenclatureLine:
    ...
    def __init__(self, lst: list) -> None:
        ...
        code_info = self.codes_info.get(self.get_code(0)) or self.codes_info.get(
            self.get_code(1)
        )
...
class Nomenclature:
    ...
    def get_time_lines(self, place, patho, time):
        return list(
            filter(
                lambda line: line.get_duration() == time,
    ...
    def get_indemnities(self, patho):
        indemnities = self.get_all_lines(
            "domiciles", "Indemnité pour les frais de déplacement du kinésithérapeute"
        )
        ...
        for pc, cc in indemnity_types:
            if pc in patho:
                return list(
                    filter(lambda line: cc in line.get_code_type(), indemnities)
                )[0]
    ...
    def get_priority(self, patho, seance_id):
        ...
        seance_breakpoints = [
            ...
            ("FB", (60, 80)),  # FB : seance 1-60, 61-80, 81-...
        ]
    ...
    def get_corresp_lines(
        self, place, patho, seance_id, duration=30, consultative=False
    ):
        ...
        if consultative:
            return (
                result
                + list(filter(lambda line: line.get_code_type() == "CONSULT", lines))[
                    :1
                ]
            )
        ...
        if seance_id == 1:
            result += list(
                filter(lambda line: line.get_code_type() == "INTAKE", all_lines)
            )[:1]
            print("result:", result)
